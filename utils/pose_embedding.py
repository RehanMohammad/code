...
class PosEmbFactory(nn.Module):
    ...
    def get_fourier_emb(self, xyz, num_channels=None, input_range=None):
        ...
        #     bsize, npoints, d_out
        # )
        flat_xyz = xyz.reshape(-1, d_in)                                  # (B*npoints, d_in)
        proj = torch.mm(flat_xyz, self.gauss_B[:, :d_out])               # (B*npoints, d_out)
        xyz_proj = proj.reshape(bsize, npoints, d_out)                   # (B, npoints, d_out)
