...
from model.mae import TemporalEdgeMAE as MAE
...
if __name__ == '__main__':
	...
	parser.add_argument('--cfg_path', default='configs/ipn_configs.yaml', help='Path to the train.yaml config')
	...
	mae = MAE(  
                ...
                edge_masking_ratio=mae_args.edge_masking_ratio,
            ).to(device)                           
	...
	train_set = FinetuningDataset(
		...
		max_seq   = stgcn_args.sequence_length,
		normalize = data_args.normalize
	)
	...
	stgcn = STGCN(
		...
		graph_args      = dict(config='IPN', labeling_mode='uniform'),
		mask_learning   = False,
		use_local_bn    = False,
		multiscale      = False,
		temporal_kernel_size = stgcn_args.temporal_kernel_size,
		dropout         = stgcn_args.dropout,
	).to(device)
