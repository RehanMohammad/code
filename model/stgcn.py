...
import numpy as np
import math
from dataset.IPN import HAND_CONNECTIONS
...
class unit_gcn(nn.Module):
    ...
    def forward(self, x, A=None):
        N, C, T, V = x.size()
        # choose which adjacency to use: passed in A (masked) or the stored default
...
class Graph():
    ...
    def __init__(self, config, labeling_mode='uniform'):
        ...
        if config == 'shrec24':
            ...
            num_node = 21
            self_link = [(i, i) for i in range(num_node)]
            inward_ori_index = HAND_CONNECTIONS
            inward = [(i, j) for (i, j) in inward_ori_index]
            outward = [(j, i) for (i, j) in inward]
            neighbor = inward + outward
...
class TCN_GCN_unit(nn.Module):
    ...
    def forward(self, x, A=None):
       x1 = self.gcn1(x, A=A)               # <— pass A into the GCN
       x1 = self.tcn1(x1)
       res = x if (self.down1 is None) else self.down1(x)
       return x1 + res
